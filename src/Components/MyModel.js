...
function MyModel({Projects,HandleAddProject,closeModal}) {
    ...
    useEffect(() => {
        ...
        .then((data) => {
            setprojectsSrc(data[Math.floor(Math.random() * 99)].download_url);
            console.log(data);
        })
    ...
    const handleSaveClick=()=>{
        if(projectsTextTitle.trim().length>0 && projectsTextDescrp.trim().length>0){
            HandleAddProject(projectsTextTitle,projectsTextDescrp,projectsSrc);
